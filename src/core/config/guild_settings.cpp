...
std::optional<std::string> guild_settings::find(dpp::snowflake guild_id,
                                                std::string_view key) const {
    auto query = db_->prepare("SELECT value FROM guild_settings WHERE guild_id = ? AND key = ?",
                              static_cast<std::uint64_t>(guild_id), key);
    if (!query.step()) {
        return std::nullopt;
    }
...
std::string guild_settings::get(dpp::snowflake guild_id, std::string_view key,
                                std::string_view fallback) const {
    return find(guild_id, key).value_or(std::string(fallback));
}

std::int64_t guild_settings::get_int(dpp::snowflake guild_id, std::string_view key,
...
double guild_settings::get_real(dpp::snowflake guild_id, std::string_view key,
...
bool guild_settings::erase(dpp::snowflake guild_id, std::string_view key) {
    db_->prepare("DELETE FROM guild_settings WHERE guild_id = ? AND key = ?",
                 static_cast<std::uint64_t>(guild_id), key)
        .run();
