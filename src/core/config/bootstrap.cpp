...
std::vector<dpp::snowflake> require_snowflakes(const json& object, std::string_view key) {
    const auto& value = object.at(std::string(key));
    if (!value.is_array()) {
        wrong_type(key, "an array of ID strings");
    }
    ...
    for (const auto& entry : value) {
        if (!entry.is_string()) {
            throw config_error("config key \"" + std::string(key) +
                               "\" must hold IDs as strings: a JSON number cannot represent a "
                               "Discord ID exactly");
        }
        ...
        try {
            ...
            throw config_error("config key \"" + std::string(key) + "\" has \"" + text +
                               "\", which is not a Discord ID");
        }
    }
    return ids;
}
...
bootstrap bootstrap::from_json(std::string_view text) {
    ...
    if (parsed.is_discarded()) {
        throw config_error("config file is not valid JSON");
    }
    if (!parsed.is_object()) {
        throw config_error("config file must contain a JSON object");
    }
    ...
    if (parsed.contains("backup_directory")) {
        config.backup_directory = require_string(parsed, "backup_directory");
    }
    if (parsed.contains("backups_to_keep")) {
        config.backups_to_keep = require_int(parsed, "backups_to_keep");
        if (config.backups_to_keep < 0) {
            throw config_error("config key \"backups_to_keep\" cannot be negative");
        }
    }
    if (parsed.contains("backup_interval_minutes")) {
        const int minutes = require_int(parsed, "backup_interval_minutes");
        if (minutes <= 0) {
            throw config_error("config key \"backup_interval_minutes\" must be positive");
        }
        config.backup_interval = std::chrono::minutes{minutes};
    }
    if (parsed.contains("llm_provider")) {
        config.llm_provider = require_string(parsed, "llm_provider");
    }
    if (parsed.contains("llm_model")) {
        config.llm_model = require_string(parsed, "llm_model");
    }
    if (parsed.contains("spend_cap_daily_usd")) {
        config.spend_cap_daily_usd = require_number(parsed, "spend_cap_daily_usd");
    }
    if (parsed.contains("spend_cap_monthly_usd")) {
        config.spend_cap_monthly_usd = require_number(parsed, "spend_cap_monthly_usd");
    }
    if (parsed.contains("llm_tool_rounds")) {
        config.llm_tool_rounds = require_int(parsed, "llm_tool_rounds");
        if (config.llm_tool_rounds < 1) {
            throw config_error("config key \"llm_tool_rounds\" must be at least 1");
        }
    }
    if (parsed.contains("trusted_guilds")) {
        config.trusted_guilds = require_snowflakes(parsed, "trusted_guilds");
    }
    if (parsed.contains("trusted_users")) {
        config.trusted_users = require_snowflakes(parsed, "trusted_users");
    }
...
secrets secrets::from_environment() {
    ...
    if (!token || token->empty()) {
        throw config_error("DISCORD_BOT_TOKEN is not set. Secrets come from the environment "
