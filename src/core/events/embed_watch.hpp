...
[[nodiscard]] auto build_edit(const edit_replacement& edit) -> dpp::message;
...
[[nodiscard]] auto embedded_links(std::span<const watched_link> links, std::span<const std::string> embed_urls) -> std::vector<bool>;
...
public:
    ...
    auto watch(watch_request request, std::span<const std::string> embed_urls = {}) -> std::vector<embed_action>;
    ...
    auto on_embeds(dpp::snowflake message_id, std::span<const std::string> embed_urls) -> std::vector<embed_action>;

    /// Moves every link whose time is up to its next attempt.
    auto tick() -> std::vector<embed_action>;
    ...
    /// hours.
    auto settle(watch_request request, std::span<const std::string> embed_urls) -> std::vector<embed_action>;

    /// Our message was deleted; there is nothing left to edit.
    auto forget(dpp::snowflake message_id) -> void;

    [[nodiscard]] auto watching(dpp::snowflake message_id) const -> bool;
...
private:
    ...
    static auto absorb(watch_state& state, std::span<const std::string> embed_urls) -> bool;

    /// The end of a watch: records the outcome and says what to change.
    auto finish(const watch_state& state) -> std::vector<embed_action>;

    auto prune_early(std::chrono::steady_clock::time_point now) -> void;
